...
class StableDiffusionImg2ImgPipelineOld(DiffusionPipeline):
    ...
    def __call__(
        ...
        censored_image, has_nsfw_concept = self.safety_checker(
            images=image.copy(), clip_input=safety_cheker_input.pixel_values
        )

        if self.censor_nsfw:
            image = self.numpy_to_pil(censored_image)
        else:
